...
- Versions below 6.21.6 are not recommended due to unstable MSR handling

## 6.21.6
- Added optional MSR mod toggle (msr_mod) so user can enable/disable MSR optimization
- Added MSR self-test inside container (rdmsr validation) to verify real MSR access
...
## 6.21.3
- Fixed Docker publishing workflow issues
